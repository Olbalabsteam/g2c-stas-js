...
async function contractWithCallback(
  ...
  if (
    ...
    throw new UtxoError("inputUtxos is invalid");
  }
  if (tokenSatoshis === 0) {
    throw new SchemaError("Token satoshis is zero");
  }
  if (publicKey === null) {
    throw new KeyError("Issuer public key is null");
  }
  ...
  /* BUG let totalSatAmount = 0;
  for (var i = 0; i < inputUtxos.length; i++) {
    totalSatAmount += inputUtxos[i].satoshis;
  }
  if (tokenSatoshis > totalSatAmount) {
    throw new SchemaError(
      `Token Supply in Satoshis of ${tokenSatoshis} is greater than input amount of ${totalSatAmount}`
    );
  }*/

  //FIX BUG, force output0 to hold full budget
  if (schema.totalSupply * schema.satsPerToken > inputUtxos[0].satoshis) {
    throw new Error(
      `Token Supply of ${tokenSatoshis} with satsPerToken of ${schema.satsPerToken} is greater than input amount of ${inputUtxos[0].amount}`
    );
  }
...
function validateSchema(schema) {
  if (schema === null) {
    throw new SchemaError("Schema is null");
  }
  ...
  if (
    ...
    throw new SchemaError("Invalid satsPerToken. Must be over 0.");
  }
  if (typeof schema.tokenId === "undefined" || schema.tokenId === null) {
    throw new SchemaError("Token id is required");
  }
  if (
    typeof schema.totalSupply === "undefined" ||
    schema.totalSupply === null
  ) {
    throw new SchemaError("Total Supply is required");
  }
}
